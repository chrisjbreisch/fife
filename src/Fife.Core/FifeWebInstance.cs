...
public sealed class FifeWebInstance : IFifeObject
{
    ...
    public object? Get(Token name) => name.Lexeme switch
    {
        ...
        "get" => new NativeFunction("get", 1, 1, (interpreter, arguments) =>
            Send(interpreter, HttpMethod.Get, RequireString(arguments[0], name, "url"), null, name)),
        "post" => new NativeFunction("post", 1, 2, (interpreter, arguments) =>
            Send(interpreter, HttpMethod.Post, RequireString(arguments[0], name, "url"), OptionalBody(arguments, name), name)),
        "put" => new NativeFunction("put", 1, 2, (interpreter, arguments) =>
            Send(interpreter, HttpMethod.Put, RequireString(arguments[0], name, "url"), OptionalBody(arguments, name), name)),
        "patch" => new NativeFunction("patch", 1, 2, (interpreter, arguments) =>
            Send(interpreter, HttpMethod.Patch, RequireString(arguments[0], name, "url"), OptionalBody(arguments, name), name)),
